/*──────────────────────────────────────────────────────────────
  ScrollContent.tsx — FULL FILE
  Adds neon glow + subtle flicker to every image (and inline-SVG)
  inside “reveal” blocks, without touching FeatureBlock itself.
──────────────────────────────────────────────────────────────*/
...
/*──────────────────────────────────────────────────────────────
  ...
  • NEW: attaches `neon` class + random animation-delay to all
    images / SVGs inside `.reveal` elements, enabling the global
    glow + flicker defined in index.css
──────────────────────────────────────────────────────────────*/
const ScrollContent = () => {
  ...
  useEffect(() => {
    ...
    const onIntersect: IntersectionObserverCallback = (entries) => {
      entries.forEach((entry) => {
        ...
        /* opacity & translateY based on visibility */
    ...
    /* ---------------- 2. NEON GLOW ATTACHER ---------------- */
    /*  – adds class once on mount; no re-runs needed           */
    document
      .querySelectorAll<HTMLImageElement | SVGElement>(".reveal img, .reveal svg")
      .forEach((graphic) => {
        /* ensure the glow + flicker rule from index.css applies */
        graphic.classList.add("neon");

        /* optional: randomise flicker phase so icons aren’t in sync */
        const rnd = Math.random() * 3;           // 0 → 3
        graphic.style.animationDelay = `-${rnd}s`;
      });
