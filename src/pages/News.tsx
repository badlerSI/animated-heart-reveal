...
const WEB_APP_URL =
  'https://script.google.com/macros/s/AKfycby8Lks5-o3Wwaj4xPqnO5a-w3HiY7JAYWTwgfYOCjHJFx8S45IuNVIUESfGMDpj3BeUMw/exec';
...
export default function News() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(url);          // simple GET (no CORS pre-flight)
  ...
  return (
    <div className="min-h-screen bg-[#0d0d12] flex flex-col items-center px-4 py-16">
      ...
      <div className="max-w-4xl text-center mb-8">
        <p className="text-lg md:text-xl text-cyan-white/90 font-outfit">
          Sign up for the Soul Dispatch and be first to know about product updates,
          car-show appearances, events, and swag giveaways!
        </p>
      </div>
      ...
      <div className="max-w-md w-full">
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <input
            ...
            required
            disabled={busy}
            className="px-4 py-3 text-base rounded bg-[#0d0d12] border border-cyan-white/30 text-cyan-white"
          />
          <button
            type="submit"
            disabled={busy}
